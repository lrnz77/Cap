  ...
  return useQuery({
    ...
    refetchInterval: (query) => {
      ...
      const shouldContinuePolling = () => {
        if (!data.transcriptionStatus || data.transcriptionStatus === "PROCESSING") {
          return true;
        }
        ...
        if (data.transcriptionStatus === "COMPLETE") {
          if (!aiGenerationEnabled) return false;
          if (data.aiProcessing) return true;
          if (!data.summary && !data.chapters) return true;
          return false;
        }
        return false;
      };
...
export const Share = ({
  ...
  const shouldShowLoading = () => {
    ...
    if (transcriptionStatus === "COMPLETE") {
      if (aiData.generationError) return false;
      if (aiData.processing === true) return true;
      if (!aiData.summary && !aiData.chapters) return true;
    }
